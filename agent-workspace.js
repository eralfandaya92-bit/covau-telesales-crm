(function setupAgentWorkspace(){
  ...
  content.insertAdjacentHTML('beforeend',`<section id="agent-workspace" class="page agent-workspace">
    ...
    <div class="agent-layout">
      ...
      <form id="agentLeadForm" class="hidden"><div class="agent-panel-scroll"><div class="agent-section"><div class="section-title">Business and contact details</div><div class="agent-detail-grid"><div><span>Business</span><b id="awBusiness">—</b></div><div><span>Phone</span><b id="awPhone">—</b></div><div><span>Industry</span><b id="awIndustry">—</b></div><div><span>Address</span><b id="awAddress">—</b></div><div><span>Last Contact</span><b id="awLastContact">—</b></div><div><span>Lead ID</span><b id="awLeadId">—</b></div></div></div>
  ...
  function deriveLeadStage(values){
    ...
    if(values.decision_maker==='Y'||['Not Interested','DNC','Head Office','Unserviceable'].includes(values.disposition))return 'Contacted';
  ...
  async function loadAgentWorkspace(){
    ...
    let q=sb.from('leads').select('id,lead_id,business_name,industry_type,phone_number,address,suburb,state,agent_name,disposition,decision_maker,lead_stage,last_contact_date,next_follow_up,remarks,invoice_status,current_retailer,tariff_code,quote_status,opportunity_stage,outcome,converted_date,created_at,updated_at').order('next_follow_up',{ascending:true,nullsFirst:false}).limit(1000);
  ...
  function visibleRows(){
    ...
    if(term)rows=rows.filter(r=>[r.business_name,r.phone_number,r.lead_id,r.suburb].some(v=>String(v||'').toLowerCase().includes(term)));
    return rows;
  }
  function renderAgentQueue(){
    ...
    byId('agentLeadQueue').innerHTML=rows.length?rows.map(r=>`<button type="button" class="agent-lead ${selectedLead?.id===r.id?'active':''}" data-agent-lead="${r.id}"><div class="agent-lead-top"><b>${esc(r.business_name||'Unnamed business')}</b>${stageBadge(deriveLeadStage(r))}</div><div class="agent-lead-meta"><span>${esc(r.phone_number||r.suburb||'No phone')}</span><span>${r.next_follow_up?'Due '+esc(r.next_follow_up):esc(r.lead_id||'')}</span></div></button>`).join(''):'<div class="empty">No leads in this queue.</div>';
  ...
  function selectAgentLead(r){
    ...
    setText('agentPanelTitle',r.business_name||'Lead Workspace');setText('agentPanelSubtitle',[r.suburb,r.state,r.phone_number].filter(Boolean).join(' · '));
    setText('awBusiness',r.business_name);setText('awPhone',r.phone_number);setText('awIndustry',r.industry_type);setText('awAddress',[r.address,r.suburb,r.state].filter(Boolean).join(', '));setText('awLastContact',r.last_contact_date);setText('awLeadId',r.lead_id);
  ...
  function updateDerivedStagePreview(){const stage=deriveLeadStage(formValues());const badge=byId('agentDerivedStage');badge.textContent=stage;badge.className='badge '+stage.toLowerCase().replaceAll(' ','')}
